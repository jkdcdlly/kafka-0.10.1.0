...
public class Sender implements Runnable {
    ...
    void run(long now) {
        ...
        // TODO 第2步 获取准备发送数据的分区列表
        RecordAccumulator.ReadyCheckResult result = this.accumulator.ready(cluster, now);
        // if there are any partitions whose leaders are not known yet, force metadata update
        // TODO 第3步 如果分区没有 leader ，更新 metadata 的 needUpdate 变量，下次循环就会重新获取元数据
        ...
        //  TODO 第4步 检查要接收 batch 的 leader 的网络连接是否已建立好
        ...
        // TODO 第5步 按照 broker 分组，同一个broker 的 partition 为一组
        // TODO 如果 partition数量 > broker数量，必然有多个 leader partition 在同一个broker上
        Map<Integer, List<RecordBatch>> batches = this.accumulator.drain(cluster,
                                                                         result.readyNodes,
                                                                         this.maxRequestSize,
                                                                         now);
        // TODO guaranteeMessageOrder 默认：false
