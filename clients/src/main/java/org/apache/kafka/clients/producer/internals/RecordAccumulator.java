...
public final class RecordAccumulator {
    ...
    public RecordAppendResult append(TopicPartition tp,
        ...
        try {
            // TODO 第一步 为指定的 分区 获取或者创建一个队列（第一次肯定是创建一个队列）
            Deque<RecordBatch> dq = getOrCreateDeque(tp);
            synchronized (dq) {
                if (closed)
                    throw new IllegalStateException("Cannot send after the producer is closed.");
                // TODO 第二步 尝试把记录添加到队列中的最后一个批次（第一次肯定没有批次，直接看后面代码）
                RecordAppendResult appendResult = tryAppend(timestamp, key, value, callback, dq);
                if (appendResult != null)
                    return appendResult;
            }
            // TODO 第三步 在 batchSize 和 recordSize 间选择一个较大的（注意：recordSize + 34）
            int size = Math.max(this.batchSize, Records.LOG_OVERHEAD + Record.recordSize(key, value));
            log.trace("Allocating a new {} byte message buffer for topic {} partition {}", size, tp.topic(), tp.partition());

            // TODO 第四步 请求分配一块内存（HeapByteBuffer）
            ByteBuffer buffer = free.allocate(size, maxTimeToBlock);
            synchronized (dq) {
                // Need to check if producer is closed again after grabbing the dequeue lock.
                if (closed)
                    throw new IllegalStateException("Cannot send after the producer is closed.");
                // TODO 第五步 再次尝试把记录添加到队列中
                RecordAppendResult appendResult = tryAppend(timestamp, key, value, callback, dq);
                // TODO 第六步 如果记录添加队列成功，则释放第四步请求的内存，并返回
                if (appendResult != null) {
                    free.deallocate(buffer);
                    return appendResult;
                }
                // TODO 第七步 构建 RecordBatch 对象，并把记录封装到 RecordBatch 里
                MemoryRecords records = MemoryRecords.emptyRecords(buffer, compression, this.batchSize);
                RecordBatch batch = new RecordBatch(tp, records, time.milliseconds());
                FutureRecordMetadata future = Utils.notNull(batch.tryAppend(timestamp, key, value, callback, time.milliseconds()));
                // TODO 第八步 把 RecordBatch 对象加入队列中
                dq.addLast(batch);
    ...
    private RecordAppendResult tryAppend(long timestamp, byte[] key, byte[] value, Callback callback, Deque<RecordBatch> deque) {
        // 获取队列最后一个 RecordBatch
        RecordBatch last = deque.peekLast();
        if (last != null) {
            // 将新记录追加到此 RecordBatch
            FutureRecordMetadata future = last.tryAppend(timestamp, key, value, callback, time.milliseconds());
            if (future == null) // 为 null 说明该 RecordBatch 已满
                last.records.close();
            else
                return new RecordAppendResult(future, deque.size() > 1 || last.records.isFull(), false);
        }
        // 返回 null 有两种情况，
        // 第一种是 队列刚创建，没有 RecordBatch，需添加 RecordBatch
        // 第二种是 RecordBatch 已满，需重新添加 RecordBatch
        return null;
    }
    ...
    public ReadyCheckResult ready(Cluster cluster, long nowMs) {
        // TODO 第1步 创建一个 Hashset ，用于存储准备发送的 Node
        Set<Node> readyNodes = new HashSet<>();
        long nextReadyCheckDelayMs = Long.MAX_VALUE;
        Set<String> unknownLeaderTopics = new HashSet<>();
        // TODO 第2步 判断内存池是否内存不足
        boolean exhausted = this.free.queued() > 0;
        // TODO 第3步 遍历 batches(ConcurrentMap) 变量
        for (Map.Entry<TopicPartition, Deque<RecordBatch>> entry : this.batches.entrySet()) {
            TopicPartition part = entry.getKey();  // 分区
            Deque<RecordBatch> deque = entry.getValue(); // 队列
            // TODO 第1步 获得分区的 leader
            Node leader = cluster.leaderFor(part); // 分区 leader
            synchronized (deque) {
                if (leader == null && !deque.isEmpty()) {
                    ...
                    if (batch != null) {
                        ...
                        // 需要等待的时间：如果是重试那就是 retryBackoffMs 否则是 lingerMs(默认0)
                        // 强烈建议配置lingerMs,否则就变成有一条就发送了，太不合理了
        ...
        // TODO 第4步 返回 ReadyCheckResult（封装了 readyNodes 对象）
    ...
    private Deque<RecordBatch> getOrCreateDeque(TopicPartition tp) {
        // 线程1：d=null
        // 线程2：d=null
        Deque<RecordBatch> d = this.batches.get(tp); // batches 类型是 CopyOnWriteMap
        if (d != null)
            return d;
        ...
        // 线程2：给d赋值
        d = new ArrayDeque<>();
        // 线程1：put 到 map，返回 null（同步）
        // 线程2：put 时，先检查是否已经包含相同key，返回已有的value(同步）
        Deque<RecordBatch> previous = this.batches.putIfAbsent(tp, d);
        // 线程1：previous == null ，所以返回 d
        // 线程2：previous != null ，所以返回 previous
        if (previous == null)
            return d;
        else
            return previous;
